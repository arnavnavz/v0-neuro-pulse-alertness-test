    ...
    <main className="min-h-screen bg-background p-4 md:p-6">
      <div className="text-center space-y-3 mb-8">
        <div className="flex items-center justify-center gap-3 mb-3">
          ...
          <h1 className="text-4xl md:text-5xl font-bold tracking-tight text-foreground">NeuroPulse</h1>
        </div>
      ...
      <div className="max-w-3xl mx-auto space-y-5">
        {testState === 'flashInstruction' && (
          <Card className="p-6 md:p-8 space-y-5 bg-card/80 backdrop-blur-sm border-2 border-border shadow-lg">
            ...
            <div className="space-y-4">
              ...
              <Button 
                ...
                size="lg"
                className="w-full h-16 text-base bg-primary hover:bg-primary/90 font-semibold rounded-xl shadow-md hover:shadow-lg transition-all"
        ...
        {testState === 'flashActive' && (
          <Card className="p-5 md:p-6 space-y-5 bg-card/80 backdrop-blur-sm border-2 border-border shadow-lg">
        ...
        {testState !== 'flashInstruction' && testState !== 'flashActive' && (
          <Card className="p-5 md:p-6 space-y-5 bg-card/80 backdrop-blur-sm border-2 border-border shadow-lg">
            ...
            <div className="relative">
              ...
              <div className="relative aspect-[4/3] bg-secondary/60 rounded-xl overflow-hidden border-2 border-border/50 shadow-inner">
                <video 
                  ref={videoRef} 
                  autoPlay 
                  playsInline 
            ...
            <div>
              <h3 className="text-sm font-semibold mb-3 text-foreground">Test Mode</h3>
              <div className="grid grid-cols-3 gap-3">
                <Button
                  ...
                  className="h-auto py-4 flex flex-col items-center gap-2 rounded-xl transition-all hover:scale-105"
                ...
                >
                  ...
                  <span className="text-xs font-semibold">Dot Grid</span>
                </Button>
                <Button
                ...
                >
                  ...
                  <span className="text-xs font-semibold">Flash Test</span>
                </Button>
              </div>
            </div>
            ...
            <div className="space-y-3">
              {testState === 'idle' && (
                <div className="text-center space-y-5 py-8">
                  ...
                  <Button 
                    onClick={startTest} 
                    size="lg"
                    className="w-full text-base h-14 md:h-16 bg-primary hover:bg-primary/90 font-semibold rounded-xl shadow-md hover:shadow-lg transition-all"
              ...
              {testState === 'flash' && testMode === 'simple' && (
                <div 
                  className="text-center py-20 cursor-pointer bg-secondary/50 rounded-xl active:bg-secondary/70 transition-all hover:scale-[1.02] border-2 border-border/30"
                  onClick={handleReaction}
                >
                  <p className="text-7xl md:text-8xl font-bold text-primary animate-pulse">
                    GO!
                  </p>
              ...
              {testState === 'flash' && testMode === 'dotgrid' && (
                <div className="relative bg-secondary/40 rounded-xl border-2 border-border/50 h-96 md:h-[28rem] shadow-inner">
                  <div className="absolute top-3 left-4 text-sm font-semibold text-foreground bg-card/80 px-3 py-1.5 rounded-lg">
                    Round {dotRound}/10
                  </div>
                  {currentDot && (
                    <button
                      onClick={handleDotClick}
                      className="absolute w-16 h-16 bg-primary rounded-full active:scale-110 transition-all cursor-pointer shadow-xl hover:shadow-2xl border-2 border-primary/20"
                      style={{
                        left: `${currentDot.x}%`,
                        top: `${currentDot.y}%`,
              ...
              {testState === 'results' && (
                <div className="space-y-5">
                  <div className="text-center py-8 bg-gradient-to-br from-secondary/60 to-secondary/30 rounded-xl border-2 border-border/50 shadow-inner">
                    <p className="text-xs text-muted-foreground mb-3 font-medium uppercase tracking-wide">
                      {(results && dotGridResults) || (results && flashTestResults) || (dotGridResults && flashTestResults)
                        ? 'Combined NeuroScore' 
                        : 'NeuroScore'}
                    </p>
                    <p className="text-6xl md:text-7xl font-bold text-foreground mb-3">
                  ...
                  {testMode === 'simple' && results && (
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-secondary/50 p-4 rounded-xl text-center border border-border/30 shadow-sm">
                        <p className="text-xs text-muted-foreground mb-2 font-medium">Reaction Time</p>
                        <p className="text-2xl md:text-3xl font-bold text-foreground">
                          {results.reactionTime}<span className="text-sm">ms</span>
                        </p>
                      </div>
                      <div className="bg-secondary/50 p-4 rounded-xl text-center border border-border/30 shadow-sm">
                        <p className="text-xs text-muted-foreground mb-2 font-medium">Movement Index</p>
                        <p className="text-2xl md:text-3xl font-bold text-foreground">
                  ...
                  {testMode === 'dotgrid' && dotGridResults && (
                    <div className="grid grid-cols-3 gap-3">
                      <div className="bg-secondary/50 p-4 rounded-xl text-center border border-border/30 shadow-sm">
                        <p className="text-xs text-muted-foreground mb-2 font-medium">Avg Time</p>
                        <p className="text-xl md:text-2xl font-bold text-foreground">
                          {dotGridResults.averageReactionTime}<span className="text-xs">ms</span>
                        </p>
                      </div>
                      <div className="bg-secondary/50 p-4 rounded-xl text-center border border-border/30 shadow-sm">
                        <p className="text-xs text-muted-foreground mb-2 font-medium">Hits</p>
                        <p className="text-xl md:text-2xl font-bold text-success">
                          {dotGridResults.hits}/10
                        </p>
                      </div>
                      <div className="bg-secondary/50 p-4 rounded-xl text-center border border-border/30 shadow-sm">
                        <p className="text-xs text-muted-foreground mb-2 font-medium">Misses</p>
                  ...
                  {testMode === 'flash' && flashTestResults && (
                    <div className="space-y-3">
                      <div className="grid grid-cols-2 gap-3">
                        <div className="bg-secondary/50 p-4 rounded-xl text-center border border-border/30 shadow-sm">
                          <p className="text-xs text-muted-foreground mb-2 font-medium">Pupil Response</p>
                          <p className="text-xl md:text-2xl font-bold text-foreground">
                            {flashTestResults.reactionTimeMs}<span className="text-xs">ms</span>
                          </p>
                        </div>
                        <div className="bg-secondary/50 p-4 rounded-xl text-center border border-border/30 shadow-sm">
                          <p className="text-xs text-muted-foreground mb-2 font-medium">Blink Latency</p>
                          <p className="text-xl md:text-2xl font-bold text-foreground">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="bg-secondary/50 p-4 rounded-xl text-center border border-border/30 shadow-sm">
                          <p className="text-xs text-muted-foreground mb-2 font-medium">Blinks</p>
                          <p className="text-xl md:text-2xl font-bold text-foreground">
                            {flashTestResults.blinkCount}
                          </p>
                        </div>
                        <div className="bg-secondary/50 p-4 rounded-xl text-center border border-border/30 shadow-sm">
                          <p className="text-xs text-muted-foreground mb-2 font-medium">Stability</p>
                          <p className="text-xl md:text-2xl font-bold text-foreground">
        ...
        {/* AI Report Card */}
        <Card className="p-5 md:p-6 space-y-4 bg-card/80 backdrop-blur-sm border-2 border-border shadow-lg">
          ...
          <div className="space-y-3">
            ...
            {/* Suggestion */}
            <div className="bg-secondary/40 border-2 border-border/50 rounded-xl p-4 space-y-2">
              <div className="flex items-center gap-2">
        ...
        <Card className="p-4 bg-secondary/30 border-2 border-border/50 rounded-xl">
